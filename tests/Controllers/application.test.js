...
test('Application Test 1 - Get All Applications', async () => {
    ...
    let result = await applicationController.getAllApplications();
...
test('Application Test 3 - Get Application by Id (no results)', async () => {
    ...
    if (DB_Conn === Constants.DB_CONNS_MONGO) { applicationModel.getApplicationById = jest.fn().mockReturnValue(null); };
...
test('Application Test 7 - Update / Delete Application (application Id not found)', async () => {
    ...
    applicationController.getApplicationById = jest.fn().mockReturnValue('not found');
